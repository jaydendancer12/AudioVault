...
function buildLikedSongsCsv(likedSongs) {
  const rows = [
    ['track_id', 'track_uri', 'name', 'artists', 'album', 'release_date', 'duration_ms', 'duration', 'explicit', 'added_at', 'spotify_url']
  ];
...
export function buildLibrarySnapshotPayload({ user, likedSongs, playlists }) {
...
export async function exportLibrarySnapshot({ user, likedSongs, playlists }) {
  const payload = buildLibrarySnapshotPayload({ user, likedSongs, playlists });
